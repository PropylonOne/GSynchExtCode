...
{
    [Serializable]
    public class SSSurveyAttributeGroupMaint : PXGraph<SSSurveyAttributeGroupMaint>
    {
        ...
        public SSSurveyAttributeGroupMaint()
        {
        ...
        public static string getEntityNameStatic(string classid)
        {
            switch (classid)
            {
                case GroupTypes.SolarSiteSurvey:
                    return typeof(SolarSiteSurvey).FullName;

                case GroupTypes.SolarSite:
                    return typeof(SolarSite).FullName;
        ...
        protected virtual void CSAttributeGroup_RowDeleting(PXCache sender, PXRowDeletingEventArgs e)
        {
            ...
            if (attributeGroup.IsActive == true)
                throw new PXSetPropertyException(PX.Objects.CR.Messages.AttributeCannotDeleteActive);

            if (Mapping.Ask("Warning", PX.Objects.CR.Messages.AttributeDeleteWarning, MessageButtons.OKCancel) != WebDialogResult.OK)
            {
                e.Cancel = true;
                return;
            }
        ...
        public partial class GroupTypeFilter : IBqlTable
        {
            ...
            [GroupTypes.List()]
            [PXDefault(GroupTypes.SolarSite)]
